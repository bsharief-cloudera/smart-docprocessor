...
def dummy_proofread(input_path, edit_path, correction_path):
    ...
    modified_doc, corrections_doc = llm_proofer.proofread_and_correct_document( modified_doc, corrections_doc)
...
def upload_file():
    ...
    if file:
        ...
        edit_path = os.path.join(app.config['UPLOAD_FOLDER'], filename.split('.')[0] + '_edited.docx')
        correction_path = os.path.join(app.config['UPLOAD_FOLDER'], filename.split('.')[0] + '_corrections.docx')
...
def proofread_document(filename):
    ...
    return f"<h3>Processing Completed in {processing_time}. Proceed to download your files.</h3>" \
...
def download_file(filename, filetype):
    file_path = os.path.join(app.config['UPLOAD_FOLDER'], filename.split('.')[0] + f'_{filetype}.docx')
    return send_file(file_path, as_attachment=True)

if __name__ == '__main__':
    app.run(host='127.0.0.1', port=int(os.getenv('CDSW_READONLY_PORT',8100)))